...
const EfectivoSobrantesEfectivoSobrantesFaltantes = ({ formData }) => {
  ...
  const getResumeningresosme = async (formData) => {
    ...
    try {
      const { data } = await Axios.post("https://oceanicadeseguros.com/asg-api/dbo/satelite/totalingresosinverme", params);
      setResult(data);
    } catch (err) {
    ...
    <>
      <CardManagement>
        <GridContainer justify="center" alignItems="center">
          <GridItem xs={12} md={8}>
            <div>
              <h1>Efectivo Sobrantes Efectivo Sobrantes Faltantes</h1>
            </div>
          ...
          <GridItem xs={12} md={4}>
            <BarChart montoMoneda={montoMoneda} montoFaltante={montoFaltante}  office={office} />
          </GridItem>
        </GridContainer>
      </CardManagement>
    </>
...
export default EfectivoSobrantesEfectivoSobrantesFaltantes;
