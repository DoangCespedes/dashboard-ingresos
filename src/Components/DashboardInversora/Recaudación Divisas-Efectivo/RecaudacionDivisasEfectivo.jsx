...
const RecaudacionDivisasEfectivo = ({ formData }) => {
  ...
  const getResumeningresosme = async (formData) => {
    ...
    try {
      const { data } = await Axios.post("https://oceanicadeseguros.com/asg-api/dbo/satelite/resumeningresosinverme", params);
      setResult(data);
    } catch (err) {
    ...
    <>
      <CardManagement>
        <GridContainer justify="center" alignItems="center">
          <GridItem xs={12} md={8}>
            <div>
              <h1>Recaudacion Divisas-Efectivo</h1>
            </div>
            ...
            {error && <div>{error}</div>}
              <div>
                <div style={{padding:"5px", width: "100%", background: "#f5f5f5", color: "#000", fontSize: "1.5rem", textAlign: "center", borderRadius: "10px" }}>
                  {montoMoneda ? <TableInfo montoMoneda={montoMoneda} office={office} /> : 'No hay datos '} {/* Manejo del caso en que montoSobrante es undefined */}
                </div>
              </div>
          </GridItem>

          <GridItem xs={12} md={4}>
            <BarChart montoMoneda={montoMoneda} office={office}/>
          </GridItem>
        </GridContainer>
      </CardManagement>
    </>
